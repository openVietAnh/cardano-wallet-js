...
export class Seed {
  ...
  static buildTransaction(
    ...
    let tbConfig = TransactionBuilderConfigBuilder.new()
      ...
      .fee_algo(
        LinearFee.new(
          toBigNum(config.protocols.txFeePerByte),
          toBigNum(config.protocols.txFeeFixed)
        )
      )
      //min-ada-value
  ...
  static buildTransactionWithToken(
    ...
    const fee = parseInt(
      opts.config.protocols.maxTxSize * opts.config.protocols.txFeePerByte +
        opts.config.protocols.txFeeFixed
  ...
  static buildTransactionMultisig(
    ...
    const selectionfee = parseInt(
      config.protocols.maxTxSize * config.protocols.txFeePerByte +
        config.protocols.txFeeFixed
    ); // 16384 * 44 + 155381 = 876277
    const currentfee =
      coinSelection.inputs.reduce((acc, c) => c.amount.quantity + acc, 0) +
      (coinSelection.withdrawals?.reduce(
  ...
  static getMinUtxoValueWithAssets(
    ...
    let min = min_ada_required(
      assets,
      false,
      toBigNum(config.protocols.utxoCostPerWord)
    );
