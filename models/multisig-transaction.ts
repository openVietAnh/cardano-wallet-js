...
export class MultisigTransaction {
  ...
  static new(
    ...
    multisig.txBody = multisig.adjustFee(
      txBody,
      coinSelection,
      tokens,
      numberOfWitnesses,
  ...
  adjustFee(
    ...
    if (txFee < bodyFee) {
      ...
      const body = TransactionBody.new(
        txBody.inputs(),
        txOutputs,
        BigNum.from_str(txFee.toString()),
        txBody.ttl()
      );
  ...
  build(): string {
    ...
    const tx = Transaction.new(this.txBody, witnesses, this.metadata);
  ...
  toBytes(): Uint8Array {
    ...
    const keys = Array.from(Array(this.vkeyWitnesses.len()).keys())
      .map((i) => this.vkeyWitnesses.get(i).to_bytes())
      .map((k) => MultisigTransaction.buff2hex(k));
    const scripts = Array.from(Array(this.nativeScripts.len()).keys())
      .map((i) => this.nativeScripts.get(i).to_bytes())
      .map((s) => MultisigTransaction.buff2hex(s));
    const data = {
  ...
  static fromBytes(bytes: Uint8Array): MultisigTransaction {
    ...
    const multisig = new MultisigTransaction();
    multisig.txBody = TransactionBody.from_bytes(
      MultisigTransaction.hexToBuff(body)
    );
    multisig.txHash = hash_transaction(multisig.txBody);
    multisig.metadata = metadata
      ? AuxiliaryData.from_bytes(MultisigTransaction.hexToBuff(metadata))
      : null;
    const vKyes: Vkeywitness[] = keys.map((k: any) =>
      Vkeywitness.from_bytes(MultisigTransaction.hexToBuff(k))
    );
    for (const key of vKyes) {
      multisig.vkeyWitnesses.add(key);
    }
    const nScripts: NativeScript[] = scripts.map((k: any) =>
      NativeScript.from_bytes(MultisigTransaction.hexToBuff(k))
    );
  ...
  private static buff2hex(buffer: Uint8Array): string {
  ...
  private fakeTx(txBody: TransactionBody, numberOfWitnesses: number) {
    ...
    const cloneMetadata = this.metadata
      ? AuxiliaryData.from_bytes(this.metadata.to_bytes())
      : null;
    const tx = Transaction.new(txBody, witnessSet, cloneMetadata);
    return tx;
  }
